...
module.exports.run = async(client, message, args) => {
    const ticket = client.settings.get(message.guild.id, `tickets.${910631696615997521}`)
    ...
    if (!message.mentions.users.first()) return message.channel.send(new client.embed().setDescription('Please mention a member to add to this ticket').setFooter(`${message.guild.name} `, message.guild.iconURL({ dynamic: true })))
    ...
    message.channel.send(new client.embed().setDescription(`${message.mentions.users.first()} Zostałeś dodany do ticketa!`).setFooter(`${message.guild.name} `, message.guild.iconURL({ dynamic: true })))
}